...
function assertArgument(argument: Argument, value: string, rawValue: string, startLine: number, startCharacter: number, endLine: number, endCharacter: number) {
    assert.equal(argument.getValue(), value);
    assert.equal(argument.getRawValue(), rawValue);
...
describe("JSON Instruction", () => {
    ...
    it("SHELL [ \"abc\" ]", () => {
        ...
        assertArgument(strings[0], "\"abc\"", "\"abc\"", 0, 8, 0, 13);
        bracket = instruction.getClosingBracket();
    ...
    it("SHELL [ \"a\", \"b\" ]", () => {
        ...
        assertArgument(strings[0], "\"a\"", "\"a\"", 0, 8, 0, 11);
        assertArgument(strings[1], "\"b\"", "\"b\"", 0, 13, 0, 16);
        bracket = instruction.getClosingBracket();
    ...
    it("SHELL [ \"a,]\" ]", () => {
        ...
        let strings = instruction.getJSONStrings();
        assert.equal(strings.length, 1);
        assertArgument(strings[0], "\"a,]\"", "\"a,]\"", 0, 8, 0, 13);
        bracket = instruction.getClosingBracket();
        assertArgument(bracket, "]", "]", 0, 14, 0, 15);
    });
    ...
    it("SHELL [ \"abc\\\\ndef\" ]", () => {
        ...
        let strings = instruction.getJSONStrings();
        assert.equal(strings.length, 1);
        assertArgument(strings[0], "\"abcdef\"", "\"abc\\\ndef\"", 0, 8, 1, 4);
        bracket = instruction.getClosingBracket();
